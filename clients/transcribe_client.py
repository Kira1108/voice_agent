...
def mic_worker(ws, loop, stream_in):
    print("🎤 Start talking... (Server will print transcriptions in its own terminal)")
    while is_running:
        try:
...
async def main():
    ...
    # Only open the input stream (no need for speaker)
    ...
    try:
        async with websockets.connect(uri) as ws:
            ...
            while is_running:
                try:
                    data = await ws.recv()
                    # Server currently doesn't send anything back, but if we later 
